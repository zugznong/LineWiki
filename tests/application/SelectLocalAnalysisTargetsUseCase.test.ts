...
describe('SelectLocalAnalysisTargetsUseCase Tests', () => {
  it('should include moves with no stored (DB) evaluations', () => {
    const policy = new StoredEvaluationPolicy();
    const useCase = new SelectLocalAnalysisTargetsUseCase(policy);
  ...
  it('should include moves with expired (too old) stored evaluations', () => {
    const policy = new StoredEvaluationPolicy({ minDepth: 18, maxAgeMs: 30 * 24 * 60 * 60 * 1000 });
    const useCase = new SelectLocalAnalysisTargetsUseCase(policy);
    const candidateMoves = [
      { uci: 'e2e4', san: 'e4' }
    ];
