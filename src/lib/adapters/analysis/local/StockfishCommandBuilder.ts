...
export class StockfishCommandBuilder {
  ...
  public static setupEngine(hashMb: number = 16, threads: number = 1): string[] {
    // 비정상 수치(NaN/음수/과대값)가 명령어에 보간되지 않도록 정수로 강제 정규화합니다.
    const safeHash = StockfishCommandBuilder.clampInt(hashMb, 1, 4096, 16);
    const safeThreads = StockfishCommandBuilder.clampInt(threads, 1, 1024, 1);
    return [
      this.uci(),
