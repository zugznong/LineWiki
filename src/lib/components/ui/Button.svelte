<script lang="ts">
  ...
  const variantStyles: Record<'primary' | 'secondary' | 'danger' | 'ghost' | 'success', string> = {
    ...
    secondary: 'bg-[var(--color-bg-card)] hover:bg-[var(--color-bg-panel)] text-slate-200 border border-[var(--color-border-primary)] px-4 py-2.5 active:scale-98',
    danger: 'bg-rose-500 hover:bg-rose-600 text-white px-4 py-2.5 shadow-lg shadow-rose-500/10 active:scale-98',
    ghost: 'bg-transparent text-slate-400 hover:bg-[var(--color-bg-panel)]/40 hover:text-slate-100 px-3 py-2',
